...
// 오프닝 패턴: 상황 중계 — 첫 쌀미음을 다 뱉는 장면
// 구성: 5항목 (직전 C4 는 4항목). A1(알러지)·B4(분유)와 겹치는 내용은 뺐다
DECKS.push({
  id: 'C5-firstfood', type: 'multi', label: '이유식 시작',
  photoDefaults: { photo: 'assets/photos/firstfood.jpg', pos: 'center', ink: true },
  slides: [
    { kind: 'photo', place: '이유식 시작', cat: '육아꿀팁',
      head: ['쌀미음 주니까 다 뱉어버려', '이유식 첫 주 챙길 5가지'] },

    { kind: 'photoBody', lead: '1. 180일에 딱 맞출 필요는 없어',
      items: ['몸무게 늘고 어른 밥 볼 때 침 흘리면 그때가 신호래',
              '150일에 시작한 집도 180일에 시작한 집도 다 있더라고'] },

    { kind: 'photoBody', lead: '2. 첫 사흘은 쌀미음만 주는 거야',
      items: ['10배죽은 쌀가루 10에 물 100 넣고 끓이면서 저어주면 돼',
              '사흘 지켜보고 괜찮으면 그때 소고기로 넘어가는 순서야'] },

    { kind: 'photoBody', lead: '3. 뱉는 건 실패가 아니라 연습이야',
      items: ['오만상 찌푸리고 다 흘려도 삼키는 법을 배우는 중이거든',
              '처음부터 넙죽 받아먹는 애가 오히려 드물다고 하더라고'] },

    { kind: 'photoBody', lead: '4. 시판 먹인다고 미안해할 것 없어',
      items: ['새벽 두세 시까지 만들다 지쳐서 시판으로 넘어간 집 많아',
              '요즘은 잘 나와서 소고기만 따로 추가하는 식으로들 하더라'] },

    { kind: 'photoBody', lead: '5. 끼니 늘리면 응가도 같이 늘어',
      items: ['한 끼에서 두 끼로 갈 때 한두 주는 원래 그렇다더라고',
              '다만 하루 일곱 번을 넘거나 기저귀에 스미면 병원으로 가자'] },
...
// ── 포스터형 목업 ──────────────────────────────────────────────
// 사진 없이 타이포그래피만으로 가는 형식. 일본 광고 레퍼런스
// (거대한 글자 · 채도 높은 단색 · 숫자가 주인공)
// 출처: 보건복지부 2027년 예산안 보도자료 및 관련 보도 (2026-09-04 기준)
DECKS.push({
  id: 'P1-support', type: 'multi', label: '2027 양육지원급여 개편(포스터형)',
  slides: [
    { kind: 'photo', bg: '#FFD400', ink: true,
      place: '양육지원급여 개편', cat: '지원금', big: '1,000{만원}',
      head: ['내년 7월생부터', '첫째가 받는 돈이야'] },

    { kind: 'photoBody', bg: '#111318',
      lead: '1. 세 가지가 두 가지로 합쳐져',
      items: ['첫만남이용권, 부모급여, 아동수당이 하나로 정리되는 거야',
              '아이맞이지원금이랑 아동기본수당, 이렇게 둘만 남는대'] },

    { kind: 'photoBody', bg: '#FF5A3C', ink: true,
      big: '1,000{만원}', lead: '2. 첫째는 출생 후 1년간',
      items: ['둘째는 1,200만원, 셋째부터는 1,500만원이라고 하더라고',
              '한 번에 주는 게 아니라 분기마다 네 번 나눠서 현금으로 준대'] },

    { kind: 'photoBody', bg: '#1E5AE8',
      big: '월 20{만원}', lead: '3. 0세부터 13세까지 매달',
      items: ['현금 10만원에 지역사랑상품권 10만원을 얹어주는 구조야',
              '지금 아동수당이 8세까지였으니까 기간이 훨씬 길어지는 거지'] },

    { kind: 'photoBody', bg: '#0E7A5F',
      big: '+500{만원}', lead: '4. 지방우대지역이면 더 얹어줘',
      items: ['해당 지역이면 첫째도 1,500만원까지 올라간다고 하더라',
              '우리 동네가 되는지는 확정되면 그때 확인해봐야겠어'] },

    { kind: 'photoBody', bg: '#111318',
      big: '7. 1', lead: '5. 2027년 7월 출생아부터야',
      items: ['1월에서 6월 사이에 태어난 아이는 아직 정해지지 않았어',
              '같은 해에 태어났는데 갈리는 건 아니라고 검토 중이라더라'] },
